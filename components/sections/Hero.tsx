...
export const Hero = () => {
  ...
  return (
    <section ref={ref} className="relative h-[100dvh] w-full flex items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: yText, opacity: opacityText }} className="relative z-30 px-4 md:px-12 w-full max-w-[1920px] mx-auto flex flex-col items-center md:items-end text-center md:text-right h-full justify-center pt-[clamp(6rem,10vh,12rem)] pb-[clamp(2rem,5vh,6rem)]">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          className="font-terminal text-gray-300 max-w-4xl md:max-w-5xl mx-auto md:mx-0 space-y-2 md:space-y-1 text-[clamp(1rem,1.1vw,1.3rem)]">
          <p className="leading-relaxed">The first cryptocurrency powered by <span className="block md:inline text-[#ffae00] font-bold md:font-normal my-1 md:my-0 text-[clamp(1.1rem,1.2vw,1.5rem)]">Proof-of-Suffering</span></p>
          <p className="text-gray-300">the only consensus mechanism traders truly understand.</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="mt-8 md:mt-12 flex flex-col md:flex-row gap-5 md:gap-8 justify-center md:justify-end items-center w-full md:max-w-4xl">
          <button onClick={handleEnterHell} className="text-gray-500 font-terminal hover:text-[#ffae00] transition-colors flex items-center gap-2 group order-2 md:order-1 text-[clamp(0.9rem,1vw,1.2rem)]">
            [ ENTER HELL ] <TrendingDown className="w-4 h-4 md:w-5 md:h-5 group-hover:translate-y-1 transition-transform" />
          </button>
          ...
          <a href={BUY_LINK} target="_blank" rel="noopener noreferrer"
            className="group relative bg-transparent border-2 border-hell-red text-hell-red font-gothic uppercase overflow-hidden transition-all hover:text-hell-white hover:border-hell-orange hover:shadow-[0_0_30px_rgba(204,0,0,0.6)] order-1 md:order-2 cursor-pointer flex items-center gap-2
                       px-[clamp(1.5rem,2vw,2.5rem)] py-[clamp(0.75rem,1vw,1.2rem)] text-[clamp(1.25rem,1.5vw,2rem)]">
