...
export const Hero = () => {
  ...
  // Refs to avoid scroll-time React state updates (prevents hook deps warning + improves smoothness)
  const dimsRef = useRef({ isMobile: false, isShort: false });
  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      dimsRef.current.isMobile = window.innerWidth < 768;
      dimsRef.current.isShort = window.innerHeight < 600;
    };
    ...
    // Initialize variables to default (static) state
    setVars({
      "--hc-contentY": "0px",
      "--hc-contentOpacity": "1",
      "--hc-titleY": "0px",
    ...
    const updateScrollProgress = () => {
      ...
      const easedProgress = progress * progress; // Quadratic ease

      // Mobile gets 60% of desktop parallax intensity, Short gets even less (30%)
      const { isMobile, isShort } = dimsRef.current;
      ...
      const contentY = progress * 200 * intensity;
      const contentOpacity = Math.max(0, 1 - easedProgress * fadeFactor);

      // Individual element stagger (each element moves at different rate)
      const titleY = progress * 180 * intensity - contentY;
      const titleOpacity = Math.max(0, 1 - easedProgress * 2.2);

      const subtitleY = progress * 220 * intensity - contentY;
      const subtitleOpacity = Math.max(0, 1 - easedProgress * 1.8);

      const taglineY = progress * 260 * intensity - contentY;
      const taglineOpacity = Math.max(0, 1 - easedProgress * 1.6);

      const ctaY = progress * 300 * intensity - contentY;
      const ctaOpacity = Math.max(0, 1 - easedProgress * 1.4);

      setVars({
        "--hc-contentY": `${contentY}px`,
        "--hc-contentOpacity": `${contentOpacity}`,
        "--hc-titleY": `${titleY}px`,
        "--hc-titleOpacity": `${titleOpacity}`,
        "--hc-subtitleY": `${subtitleY}px`,
        "--hc-subtitleOpacity": `${subtitleOpacity}`,
        "--hc-taglineY": `${taglineY}px`,
        "--hc-taglineOpacity": `${taglineOpacity}`,
        "--hc-ctaY": `${ctaY}px`,
        "--hc-ctaOpacity": `${ctaOpacity}`,
        "--hc-overlayOpacity": `${Math.max(0.3, 1 - progress * 2)}`,
      });
    };
    ...
    // Ensure refs are current before first calc
    updateDimensions();
  ...
  return (
    ...
    >
      ...
      <div
        className="relative z-10 mx-auto flex w-full max-w-[100rem] flex-col items-center px-4 pt-8 text-center short-landscape:pb-8 short-landscape:pt-20 md:items-end md:px-12 md:pt-0 md:text-right 3xl:max-w-[125rem] 3xl:pr-[20%]"
        style={
          shouldAnimate
            ? {
                transform: "translateY(var(--hc-contentY))",
                opacity: "var(--hc-contentOpacity)",
      ...
      >
        ...
        <h1
          ...
          style={
            shouldAnimate
              ? {
                  transform: "translateY(var(--hc-titleY))",
                  opacity: "var(--hc-titleOpacity)",
                  transition: smoothTransition,
                }
              : undefined
          }
        ...
        <div
          ...
          style={
            shouldAnimate
              ? {
                  transform: "translateY(var(--hc-subtitleY))",
                  opacity: "var(--hc-subtitleOpacity)",
                  transition: smoothTransition,
                }
              : undefined
          }
        ...
        <p
          ...
          style={
            shouldAnimate
              ? {
                  transform: "translateY(var(--hc-taglineY))",
                  opacity: "var(--hc-taglineOpacity)",
                  transition: smoothTransition,
                }
              : undefined
          }
        >
          WHEN THE MARKET BURNS, <span className="block md:inline">WE TREND</span>
        </p>
        ...
        <div
          ...
          style={
            shouldAnimate
              ? {
                  transform: "translateY(var(--hc-ctaY))",
                  opacity: "var(--hc-ctaOpacity)",
                  transition: smoothTransition,
                }
              : undefined
          }
