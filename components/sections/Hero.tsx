...
export const Hero = () => {
  ...
  return (
    <section ref={ref} className="relative h-screen w-full flex items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: yText, opacity: opacityText }} className="relative z-10 px-4 md:px-12 max-w-[1600px] w-full mx-auto flex flex-col items-center md:items-end text-center md:text-right">
        <motion.h1 initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", duration: 1.5 }} className="font-gothic text-6xl md:text-9xl leading-[0.9] text-hell-white text-glow drop-shadow-2xl mb-8 md:max-w-6xl">
          BORN IN THE <span className="text-hell-red">RED.</span><br />
          FORGED BY <span className="text-[#ffae00]">REGRET.</span>
        </motion.h1>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="font-terminal text-xl md:text-3xl text-gray-300 max-w-4xl md:max-w-5xl mx-auto md:mx-0 space-y-2 md:space-y-0">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0 }} className="font-terminal text-hell-red text-lg md:text-2xl mt-8 tracking-widest uppercase animate-pulse md:max-w-5xl">
          WHEN THE MARKET BURNS, WE TREND
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="mt-12 flex flex-col md:flex-row gap-8 justify-center md:justify-end items-center w-full md:max-w-5xl">
