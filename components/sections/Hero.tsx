...
export const Hero = () => {
  ...
  const handleEnterHell = () => {
    ...
    if (genesisSection) genesisSection.scrollIntoView({ behavior: "smooth" });
  };
  ...
  return (
    <section ref={ref} className="relative h-[100dvh] w-full flex items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: yText, opacity: opacityText }} className="relative z-30 px-4 md:px-12 pt-32 pb-12 w-full max-w-[1920px] mx-auto flex flex-col items-center md:items-end text-center md:text-right h-full justify-center">
        <div className="flex flex-col items-center">
            ...
            <motion.h1 initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", duration: 1.5, delay: 0.3 }}
              className="font-gothic leading-[0.9] text-hell-white text-glow drop-shadow-2xl mb-6 md:mb-8 md:max-w-7xl text-center
                         text-6xl sm:text-7xl md:text-8xl lg:text-8xl xl:text-9xl 2xl:text-[10rem]">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          ...
          <p className="leading-relaxed">The first cryptocurrency powered by <span className="block md:inline text-[#ffae00] font-bold md:font-normal my-1 md:my-0 text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl 2xl:text-5xl">Proof-of-Suffering</span></p>
          <p className="text-gray-300">the only consensus mechanism traders truly understand.</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="mt-8 md:mt-12 xl:mt-16 flex flex-col md:flex-row gap-5 md:gap-8 justify-center md:justify-end items-center w-full md:max-w-4xl">
          <button onClick={handleEnterHell} className="text-gray-500 font-terminal hover:text-[#ffae00] transition-colors flex items-center gap-2 group order-2 md:order-1 text-base sm:text-lg md:text-xl lg:text-xl xl:text-2xl 2xl:text-3xl">
            [ ENTER HELL ] <TrendingDown className="w-4 h-4 md:w-5 md:h-5 2xl:w-6 2xl:h-6 group-hover:translate-y-1 transition-transform" />
          </button>
          <a href={BUY_LINK} target="_blank" rel="noopener noreferrer"
            className="group relative bg-transparent border-2 border-hell-red text-hell-red font-gothic uppercase overflow-hidden transition-all hover:text-hell-white hover:border-hell-orange hover:shadow-[0_0_30px_rgba(204,0,0,0.6)] order-1 md:order-2 cursor-pointer flex items-center gap-2
                       px-6 py-2 text-xl sm:px-8 sm:py-3 sm:text-2xl md:px-10 md:py-4 md:text-3xl xl:px-12 xl:py-5 xl:text-4xl 2xl:px-16 2xl:py-6 2xl:text-5xl">
